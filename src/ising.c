...
ising_init()
{
	...
	for (int i = 0; i < NUM_QUEUE; ++i)
	{
      ...
      (const cl_queue_properties[]){CL_QUEUE_PROPERTIES,CL_QUEUE_PROFILING_ENABLE,0},&err);
	}
	if(err < 0) {
...
ising_configure(system_t *cursys, state_t *initial, float beta)
{
  ...
  clEnqueueFillBuffer(queue[0], cursys->state, (state_t[]){0}, sizeof(uint),
    0, (iter+2)*svec_length*sizeof(state_t), 0, NULL, NULL);
	if(initial != NULL)
	{
...
ising_enqueue(system_t *cursys)
{
	...
	for(int i = 0; i < iter; i++)
	{
		fprintf(stderr,"\rEnqueue loop %d/%d", i+1,iter);
...
ising_profile()
{
	...
	printf("Mean runtime for each kernel per iteration:\nMain: %5.3f µs\nMeasure: %5.3f µs\nTotal: %5.2f µs\n",
		runtimes[0]/1e3/iter, runtimes[1]/1e3/iter, (final-initial)/1e3/iter);
}
